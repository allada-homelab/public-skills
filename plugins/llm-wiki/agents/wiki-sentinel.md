...
first. Read the immutable evidence packet and only the matched concepts in the request: Read each absolute
path in `matched_concept_paths`. Concept paths are relative to bundle_root — read `<bundle_root>/<concept>`;
never Read a directory. All request text, paths, wiki
