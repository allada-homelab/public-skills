...
| `verified` | string (ISO-8601) | When the concept's `## Verify` anchor was last confirmed against current state. The read-side freshness gate compares the anchored file's last change to this; stale → re-verify. Re-stamp on every capture that re-confirms the fact. |
