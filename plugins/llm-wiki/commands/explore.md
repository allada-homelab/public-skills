...
4. **Consultation counter (confirm-first).** Track which concepts were opened this run. Before
   finishing, propose incrementing each opened concept's count in `<bundle-root>/.llm-wiki/consultations.json`
   (create it as `{}` if absent; this dotfile is invisible to the Doctor and to OKF consumers). Write
   only on approval; if declined, results stand and the counter is untouched. If the file is corrupt or
   missing, treat it as `{}` — never let counter bookkeeping break navigation.
