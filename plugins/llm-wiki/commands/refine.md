...
4. **Stage in a mirror.** `mirror=$(mktemp -d)`; `cp -r "<bundle>/." "$mirror/"`. Write the revised
   concept into the mirror at its path, then regenerate indexes and append the log (one UTC date,
   reused in step 7):
...
7. **Confirm & apply.** On approval, land *exactly the gated bytes* — copy the staged concept back
   ...
   --date <today>` against `<bundle>` (deterministic), and run the Doctor on the real bundle to confirm
