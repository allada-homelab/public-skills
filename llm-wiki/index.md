...
* [Config that steers a security guard's scope is untrusted input — validate in the resolver, not the UI](./guard-scope-config-is-untrusted-input.md) — When a guard derives what it protects from configurable input, a committed/shared config is attacker-controllable; enforce provenance + containment where the guard reads it, not only in the prompt that writes it.
